...
int main(int argc, char *argv[])
{
	...
	if (!is_seperate_header) {
		...
		if (image_header_size < size) {
			...
			sprintf(system_cmd,"cat %s %s.filler > %s",
					img_header_name, img_header_name, out_path_name);
			image_fill_header = true;
		} else {
	...
	if (image_fill_header) {
		snprintf(system_cmd, sizeof(system_cmd) - 1,
				 "mv %s.filler %s", curr_path_dir, out_path_dir);
		printf("system_cmd = %s\n", system_cmd);
