...
gen2_secure_image() {
  ...
  if [ "is${CONFIG_BERLIN_AS470_A0}" = "isy" ]; then
    exec_cmd=${opt_basedir_security_tools}/bin/gen_secure_image
  else
  ...
  # option only valid for AS470, need refine
  if [ "x${opt_workdir_security_keys}" != "x" ]; then
    exec_args="${exec_args} --workdir-security-keys=${opt_workdir_security_keys}"
  fi
...
if [ "is${CONFIG_BERLIN_AS470_A0}" = "isy" ]; then
  cmd_args="${cmd_args} --basedir-security-keys=${opt_basedir_security_keys}"
  cmd_args="${cmd_args} --basedir-security-tools=${opt_basedir_security_tools}"
else
