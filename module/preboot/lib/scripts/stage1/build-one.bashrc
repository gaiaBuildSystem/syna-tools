...
if [ "x${opt_board_memory_multiple}" != "x" ]; then
  cmd_args="${cmd_args} --board-memory-multiple=${opt_board_memory_multiple}"
fi
...
if [ "is$1" = "is1" ]; then
  if [ "is${opt_open_tee}" = "isy" ]; then
    build_pack_install_preboot ree
  else
    build_pack_install_preboot tee
  fi
elif [ "is$1" = "is2" ]; then
  build_pack_install_preboot $2 $3
else
  echo "Need not pack for stage $1, Only need pack for stage 1 or 2"
fi
