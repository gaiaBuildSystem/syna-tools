...
build_uboot() {
  ...
  if [ "x${CONFIG_BL_RPMB_AVB}" == "xy" ]; then
    ...
    echo "CONFIG_SYNA_FASTBOOT_RPMB_LOCK=n" > $tmpfile
  fi
  if [ "x${CONFIG_WAKEUP_INTR}" != "x" ]; then
    echo "CONFIG_SYS_WAKEUP_INTR=${CONFIG_WAKEUP_INTR}" >> $tmpfile
  fi
  if [ "x${CONFIG_STD_ANDROID_IMAGE}" != "x" ]; then
